...
class HabitViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if serializer.validated_data.get('reward') and serializer.validated_data.get('related_habit'):
        ...
        if serializer.validated_data.get('related_habit') and not serializer.validated_data[
            'related_habit'].is_pleasant:
            raise ValidationError("Связанная привычка должна быть отмечена как приятная.")
        if serializer.validated_data.get('is_pleasant') and (
                serializer.validated_data.get('reward') or serializer.validated_data.get('related_habit')):
